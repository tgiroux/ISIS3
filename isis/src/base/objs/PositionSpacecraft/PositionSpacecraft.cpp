...
namespace Isis {
  ...
  PositionSpacecraft::PositionSpacecraft(int targetCode, int observerCode,
                                         const LightTimeCorrectionState &ltState,
                                         const Distance &radius) :
                                         Position(targetCode, observerCode, ltState.isObserverTargetSwapped()) {
   ...
   */
  double PositionSpacecraft::getRadiusLightTime() const {
   ...
   *
   */
  double PositionSpacecraft::getDistanceLightTime(const Distance &distance) {
   ...
   */
  void PositionSpacecraft::SetAberrationCorrection(const QString &correction) {
    Position::SetAberrationCorrection(correction);  // Checks for validity
   ...
   * @author 2012-10-29 Kris Becker
   */
  QString PositionSpacecraft::GetAberrationCorrection() const {
   ...
   */
  void PositionSpacecraft::SetEphemerisTime(double et) {
    p_et = et;
    ...
    if ( !(m_abcorr.isLightTimeCorrected() && m_abcorr.isLightTimeToSurfaceCorrected()) ) {
      Position::SetEphemerisTimeSpice();
      return;
    }
   ...
   */
    const LightTimeCorrectionState &PositionSpacecraft::getLightTimeState() const {
      return (m_abcorr);
    }
}
