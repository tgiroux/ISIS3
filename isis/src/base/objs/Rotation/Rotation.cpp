...
namespace Isis {
  ...
  /**
   * Construct an empty Rotation class
   *
  ...
  /**
   * Construct an empty Rotation class using a valid Naif frame code to
   ...
   * @param frameCode Valid naif frame code.
   */
  Rotation::Rotation(int frameCode) : Rotation() {
    p_constantFrames.push_back(frameCode);    
  }


  /**
   * Construct an empty Rotation object using valid Naif frame code and.
   ...
   */
  Rotation::Rotation(int frameCode, int targetCode) {
  ...
  /**
   * Construct a Rotation object by copying from an existing one.
   *
   * @param rotToCopy const reference to other Rotation to copy
   */
  Rotation::Rotation(const Rotation &rotToCopy) {
...
//    for (std::vector<double>::size_type i = 0; i < rotToCopy.p_coefficients[0].size(); i++)
  ...
  /**
   * Destructor for Rotation object.
   */
  Rotation::~Rotation() {
  }
   ...
   */
  void Rotation::SetFrame(int frameCode) {
   ...
   */
  void Rotation::SetTimeBias(double timeBias) {
    p_timeBias = timeBias;
  }
   ...
   */
  void Rotation::SetEphemerisTime(double et) {
...
//   p_quaternion.Set ( p_CJ );
   ...
   */
  void Rotation::MinimizeCache(DownsizeStatus status) {
   ...
   * @throws IException::Programmer "A Rotation cache has already men
   */
  void Rotation::LoadCache(double startTime, double endTime, int size) {
   ...
   */
  void Rotation::LoadCache(double time) {
    LoadCache(time, time, 1);
  }

  void Rotation::LoadCache(json &isdRot){
      if (p_source != Spice) {
   ...
   *                                 Rotation table"
   */
  void Rotation::LoadCache(Table &table) {
    ...
    // coefficient table for angle1, angle2, and angle3
      ...
      QString msg = "Expecting either three, five, or eight fields in the Rotation table";
      throw IException(IException::Programmer, msg, _FILEINFO_);
    }
  }
   ...
   * range.
   *
   * @throws IException::Programmer "The Rotation has not yet been fit to a function"
   */
  void Rotation::ReloadCache() {
    // Save current et
    double et = p_et;
    p_et = -DBL_MAX;
    ...
    // Clear existing matrices from cache
      ...
      QString msg = "The Rotation has not yet been fit to a function";
      throw IException(IException::Programmer, msg, _FILEINFO_);
    }
   ...
   */
  Table Rotation::LineCache(const QString &tableName) {
   ...
   * @return @b Table Table with given name that contains the cached pointing
   */
  Table Rotation::Cache(const QString &tableName) {
   ...
   */
  void Rotation::loadPCFromSpice(int centerBody) {
    NaifStatus::CheckErrors();
   ...
   */
  void Rotation::loadPCFromTable(const PvlObject &label) {
    NaifStatus::CheckErrors();
 ...
 // End section added 06-20-2015 DAC
   ...
   *                           beyond PolyFunction.
   */
  void Rotation::SetPolynomial(const Source type) {
    NaifStatus::CheckErrors();
    std::vector<double> coeffAng1, coeffAng2, coeffAng3;
...
// Fix 180/-180 crossovers on angles 1 and 3 before doing fit.
   ...
   *   @history 2012-05-01 Debbie A. Cook - Added type argument to allow other function types.
   */
  void Rotation::SetPolynomial(const std::vector<double> &coeffAng1,
   ...
   */
  void Rotation::usePckPolynomial() {
   ...
   */
  void Rotation::setPckPolynomial(const std::vector<Angle> &raCoeff,
   ...
   */
  void Rotation::GetPolynomial(std::vector<double> &coeffAng1,
   ...
   */
  void Rotation::getPckPolynomial(std::vector<Angle> &raCoeff,
   ...
   */
  double Rotation::DPolynomial(const int coeffIndex) {
    double derivative;
   ...
   * @return @b double The derivative evaluated at the current time.
   */
  double Rotation::DPckPolynomial(Rotation::PartialType partialVar,
 ...
 // Number of 24 hour days in a Julian century = 36525
   ...
   */
  std::vector<double> Rotation::ToReferencePartial(std::vector<double> &lookJ,
                                                        Rotation::PartialType partialVar,
                                                        int coeffIndex) {
    NaifStatus::CheckErrors();
   ...
   */
  void Rotation::SetPolynomialDegree(int degree) {
   ...
   *
   * @return @b Rotation::Source The source of the rotation.
   */
  Rotation::Source Rotation::GetSource() {
    return p_source;
  }
   ...
   */
  void Rotation::SetSource(Source source) {
    p_source = source;
    return;
  }
  ...
  void Rotation::LoadTimeCache() {
    ...
    // Get number of ck loaded for this rotation.  This method assumes only one Rotation
    // object is loaded.
    NaifStatus::CheckErrors();
...
//            int startnoff = start1off + nints - 1;
   ...
   */
  void Rotation::FrameTrace(double et) {
    NaifStatus::CheckErrors();
    ...
    while (frameCodes[frameCodes.size()-1] != J2000Code) {
      ...
      // Rotation enumerated FrameType.  The Rotation FrameType differentiates between
   ...
   * @param et Ephemeris time.
   */
  void Rotation::InitConstantRotation(double et) {
    FrameTrace(et);
   ...
   *           |_                     _|
   *
   * @throws IException::Programmer "The Rotation pointing angles must be fit to polynomials
   ...
   *                                 polynomials"
   */
  void Rotation::ComputeAv() {
    NaifStatus::CheckErrors();

    // Make sure the angles have been fit to polynomials so we can calculate the derivative
    if (p_source < PolyFunction ) {
      QString msg = "The Rotation pointing angles must be fit to polynomials in order to ";
...
// (SpiceDouble (*) [3]) &p_CJ[0]
   ...
   */
  void Rotation::checkForBinaryPck() {
   ...
   * planetary constants.  See Rotation.h to see the valid frame types.
   */
  void Rotation::setFrameType() {
   ...
   *
   * @see Rotation::SetEphemerisTime
   */
  void Rotation::setEphemerisTimeMemcache() {
...
//         p_CJ = p_quaternion.ToMatrix();
   ...
   * @see Rotation::SetEphemerisTime
   */
  void Rotation::setEphemerisTimeNadir() {
   ...
   * @see Rotation::SetEphemerisTime
   */
  void Rotation::setEphemerisTimeSpice() {
   NaifStatus::CheckErrors();
   ...
   */
  std::vector<double> Rotation::EvaluatePolyFunction() {
   Isis::PolynomialUnivariate function1(p_degree);
   Isis::PolynomialUnivariate function2(p_degree);
   Isis::PolynomialUnivariate function3(p_degree);
   ...
   */
  void Rotation::setEphemerisTimePolyFunction() {
   NaifStatus::CheckErrors();
   Isis::PolynomialUnivariate function1(p_degree);
   Isis::PolynomialUnivariate function2(p_degree);
   Isis::PolynomialUnivariate function3(p_degree);
   ...
   * @see Rotation::SetEphemerisTime
   */
  void Rotation::setEphemerisTimePolyFunctionOverSpice() {
    setEphemerisTimeMemcache();
    NaifStatus::CheckErrors();
   ...
   * @see Rotation::SetEphemerisTime
   */
  void Rotation::setEphemerisTimePckPolyFunction() {
    // Set the scales
